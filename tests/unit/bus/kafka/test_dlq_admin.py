...
from eventsource.bus.kafka.dlq import KafkaDLQAdmin  # noqa: E402
...
class TestGetMessageCount:
    async def test_sums_end_minus_beginning_offsets_across_partitions(self) -> None:
        ...
        with patch("eventsource.bus.kafka.dlq.AIOKafkaConsumer", return_value=mock_consumer):
...
class TestDlqConsumerHelper:
    async def test_always_stops_consumer_even_when_body_raises(self) -> None:
        ...
        with (
            patch("eventsource.bus.kafka.dlq.AIOKafkaConsumer", return_value=mock_consumer),
