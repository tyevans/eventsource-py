...
| `ports/migration/` | (none -- stdlib + pydantic only) | `models.py` (`Migration`, `MigrationConfig`, `TenantRouting`, `PositionMapping`, `MigrationAuditEntry`, and their enums) plus `repositories.py` (`MigrationRepository`, `TenantRoutingRepository`, `PositionMappingRepository`, `MigrationAuditLogRepository` Protocols). Cut out of the sqlalchemy-backed `migration/repositories/` modules by ADR 0034 -- the Protocol classes were already pure in shape, just colocated with their implementations. |
| `ports/lifecycle.py` | (none -- stdlib only) | `SupportsClose` Protocol -- optional resource-release capability (ADR 0037). |
...
| *(`migration/` -- dissolved)* | -- | ADR 0034 split the package three ways: `application/migration/` (orchestration -- `coordinator.py`, `cutover.py`, and the rest, still importing sqlalchemy transitively through `adapters/sql/migration/`), `adapters/sql/migration/` (the four repository *implementations*, sqlalchemy plus `adapters/_sql/connection`'s `sql_connection`, exactly the `migration/repositories/` blocker this row used to describe, same shape, new address), and a new `ports/migration/` subpackage (`models.py` plus the four repository *Protocols*, cut out of the adapter modules -- Tier 0 in full, no sqlalchemy anywhere). `cutover.py` and `coordinator.py` still type-hint `PostgreSQLLockManager` `TYPE_CHECKING`-only, unchanged by the split. |
...
11. **Resolved by ADR 0026: `repositories/__init__.py` no longer exists.** *(Historical.)* The package `__init__` used to be 97 lines, and every line of executable code in it was an eager `from ... import` at module scope: `checkpoint`, `dlq`, `outbox`, and `eventsource.serialization`, followed by a flat 23-name `__all__` -- no `__getattr__`, no `TYPE_CHECKING` guard, no deferred import anywhere. Splitting `checkpoint.py`, `dlq.py`, and `outbox.py` into interface + in-memory and SQL halves (finding 2) did not by itself unblock this file; the `__init__` itself had to stop eagerly re-exporting the SQL-backed names. It never got narrowed or made lazy, because the whole package was deleted instead once outbox landed, which is a stronger fix than either option this finding used to weigh: there is no longer a namespace whose mere touch runs three module bodies and imports sqlalchemy, and the JSON-utility re-export (`EventSourceJSONEncoder`, `json_dumps`, `json_loads`) is gone with it -- callers reach `eventsource.serialization` directly, at the cost of orjson alone.
    ...
    The top-level package's own cost, which this finding used to trace up from `repositories/__init__.py`, no longer exists as of ADR 0035: `src/eventsource/__init__.py` no longer imports from `eventsource.repositories` at all (that package is gone), and it no longer imports `eventsource.adapters._sql.engine` or `eventsource.adapters.postgresql` at module level either -- both moved behind a PEP 562 `__getattr__` that resolves and caches each name on first access. `import eventsource` no longer requires sqlalchemy at all; deleting `repositories/` removed one route to that cost, and the lazy front door removed the cost itself. See finding 12 and cleanup step 5, both updated accordingly.

12. **Resolved by ADR 0035: the ring packages -- `domain/`, `ports/`, `application/`, `adapters/memory/` -- are Tier 0 in full, and the top-level front door no longer defeats that.** Per module, the new hexagonal core is exactly what this document asks for: every file under those four packages, `__init__`s included, resolves to stdlib, pydantic, and Tier 0 in-library modules. The legacy `stores/` package is gone entirely, so there is no longer any transitional re-export seam to describe: `EventPublisher` is defined in `ports/bus.py` and nothing else claims it. The complete import closure of the `ports` subpackage, package `__init__`s included, is `domain/`, `events/`, `exceptions.py`, and `types.py`; verified by importing every `ports` submodule under a stubbed top-level package and asserting sqlalchemy absent from `sys.modules`. The Tier 0 import-linter contract now lists `eventsource.ports` as a whole (not just `ports/snapshots`), so a reintroduced outward import would break the build.

    The caveat this finding used to record -- that `import eventsource.<anything>` executes the top-level `eventsource/__init__.py` first, and that front-door initializer eagerly reached `adapters/postgresql/snapshots.py`, so a bare runtime `import eventsource.ports` still loaded sqlalchemy regardless of which ring-Tier-0 name was actually wanted -- is closed. `src/eventsource/__init__.py` is now a PEP 562 lazy front door (ADR 0035): every public name is imported and cached on first `__getattr__` access rather than at module load, and a `TYPE_CHECKING` block carries the same import statements for static analysis. `import eventsource, sys; assert "sqlalchemy" not in sys.modules` now holds for a bare import, and the "tempting automated check" the verification section below used to say "currently fails for every module in the package" now succeeds for anything reached exclusively through the four ring packages, at last making the runtime `sys.modules` check this document asked for directly possible instead of only the static `ast`-based approximation `tests/unit/ports/test_readmodels_port_surface.py` had to fall back on.
