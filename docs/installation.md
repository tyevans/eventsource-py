...
**`dev`** is the contributor toolchain: `pytest>=8.0`, `pytest-asyncio>=0.23`, `pytest-cov>=4.0`, `pytest-benchmark>=4.0.0`, `mypy>=1.8`, `ruff>=0.14.8`, `testcontainers>=4.0`, and `pre-commit>=4.0`. Two notes. `pre-commit` is included here rather than assumed as a global install, so `uv run pre-commit install` works straight after a sync. And `bandit` is *not* in this extra -- it lives in the `[dependency-groups] dev` group alongside a tighter `mypy>=1.19.0` pin, and the pre-commit hook installs `bandit[toml]` itself. `uv sync` picks up dependency groups automatically; `pip install -e ".[dev]"` does not.
