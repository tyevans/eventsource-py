...
- The interface-adapters ring's `PostgreSQLEventStore` outbox write path (Decision 13) is the one piece of legacy-adjacent code that grows rather than shrinks in this retirement; everything else in the deletion list (§7 of the design spec) is net removal.
