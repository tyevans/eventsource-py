...
**The catch-up runner calls `handle_batch()` when your subscriber has one; the live runner does not yet.** `SubscriptionManager`'s catch-up path detects batch capability with `supports_batch_handling()` once per subscriber and, if present, delivers each read batch through `handle_batch()` as a unit — the whole batch settles (or a `handle_batch()` that raises falls back to per-event delivery through `handle()`) before the position advances, preserving the one-position-per-subscription invariant ([ordered delivery](../adrs/0059-ordered-subscription-delivery.md)). The live runner still delivers one event at a time — batching the live path is tracked separately. It calls `handle()` whenever your subscriber has one, and falls back to `handle_batch([event])` (a one-event batch) only for a `BatchSubscriber`-only subscriber that has no `handle()` at all. So a subscriber implementing exactly the published `BatchSubscriber` Protocol *does* receive live events; it simply receives them one per call rather than in real batches. A subscriber implementing neither method is rejected at runner construction with a `TypeError`, rather than failing per event once it goes live.
