...
**The catch-up runner calls `handle_batch()` when your subscriber has one; the live runner does not yet.** `SubscriptionManager`'s catch-up path detects batch capability with `supports_batch_handling()` once per subscriber and, if present, delivers each read batch through `handle_batch()` as a unit — the whole batch settles (or a `handle_batch()` that raises falls back to per-event delivery through `handle()`) before the position advances, preserving the one-position-per-subscription invariant ([ordered delivery](../adrs/DRAFT-ordered-subscription-delivery.md)). The live runner still delivers one event at a time through `handle()` regardless of `handle_batch()`'s presence — batching the live path is tracked separately. Either way, `SubscriptionConfig.batch_size` sizes the *read* batch pulled from the event store; on catch-up that read batch is now also the unit handed to `handle_batch()`, but on live it still isn't a batch handed to your subscriber. Keep `handle()` correct regardless of whether you also implement `handle_batch()` — a `BatchSubscriber`-only subscriber (no `handle()`) will not receive live events, and any subscriber driven outside `SubscriptionManager` — a backfill script, a rebuild job, a test harness feeding it `store.read_all()` output — calls whichever method it chooses.
