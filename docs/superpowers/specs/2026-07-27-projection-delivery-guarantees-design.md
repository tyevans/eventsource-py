...
The primary key serves as both the lookup index and a backstop unique
constraint making concurrent double-processing impossible rather than merely
unlikely.
...
**Ledger read before handler dispatch, not constraint-violation detection.**
Relying on the insert's unique-constraint violation would require
distinguishing our constraint from a handler's own inside a rolled-back
transaction -- indistinguishable without driver-specific error parsing, and
getting it wrong converts a genuine handler bug into a silently swallowed
"already processed." The read is one primary-key lookup.
...
1. Ledger read -- on hit, skip handler, commit, return `SKIPPED_DUPLICATE`
2. Handler execution
3. Ledger insert
4. Projection checkpoint (`update_checkpoint`)
5. Subscription position (`save_position`) -- supplied by the runner
