...
- **An event may no longer name an aggregate other than the one emitting it; doing so raises `AggregateIdMismatchError`.** `aggregate_id` was the one auto-populated field an explicit keyword argument could override, and unlike a divergent `aggregate_type` — which gets restamped — the overridden id survived to the store, where it decides which stream the event lands in. The event was then appended to a stream that disowns it: the emitting aggregate never reads that stream, the aggregate the event names never receives it, and a save/load round-trip of either one is internally consistent, so no test could see the disagreement. The guard reads `event.aggregate_id` rather than a per-aggregate declaration of permitted targets, so it applies to every aggregate with no opt-in; it fires from `apply_event(event, is_new=True)` (which catches hand-constructed events too) and from `create_event()` and `DeciderAggregate._stamp()` ahead of it, where the message can name the command being handled. Replay (`is_new=False`) is not checked. The usual source is a command that names a target (`ShipOrder(order_id=...)`) whose id is copied onto the event — that id selects *which* aggregate to load; load it and emit from it, which also gets the target's version lineage and optimistic-concurrency check right. No shim, per the pre-1.0 no-shim policy. Recorded in `docs/adrs/0065-an-event-cannot-name-another-aggregate.md`, which extends ADRs 0046 and 0048.
- **Span-attribute constants with no emitter are deleted from `eventsource.observability.attributes`** (and from the `eventsource.observability` re-export): `ATTR_ACTOR_ID`, `ATTR_STREAM_ID`, `ATTR_FROM_VERSION`, `ATTR_DB_NAME`, `ATTR_SUBSCRIPTION_STATE`, `ATTR_EVENTS_SKIPPED`, `ATTR_READMODEL_OPERATION`, `ATTR_MIGRATION_EVENTS_COPIED`, `ATTR_MIGRATION_EVENTS_TOTAL`, `ATTR_MIGRATION_PROGRESS_PERCENT`, `ATTR_MIGRATION_SYNC_LAG_MS`. Nothing in the tree ever set any of them. A span attribute is the cleanest instance of the declared-but-never-dispatched shape, because a user never sets one -- if the library does not emit the key, nobody does, and the constant reads as a contract whose dashboard query returns nothing forever with no error to explain why. ADR 0016 stated that the constant list "doubles as the catalogue of what this library will put on a span"; that is now true. Deleting rather than implementing is deliberate: inventing an emission site to justify a key nobody asked for adds unowned, untested surface to satisfy a declaration that was itself the mistake. No shim, per the pre-1.0 no-shim policy.
- **The PostgreSQL lock manager's span attribute keys change from `lock.*` to `eventsource.lock.*`.** `PostgreSQLLockManager` set `"lock.key"`, `"lock.id"`, and `"lock.timeout"` as hand-written literals while `attributes.py` declared the same three facts as `ATTR_LOCK_KEY`/`ATTR_LOCK_ID`/`ATTR_LOCK_TIMEOUT` with an `eventsource.` prefix -- two spellings of one fact, disagreeing, and invisible to a grep for the constant name. The literals are replaced by the constants, restoring the collision-safe prefix ADR 0016 requires for every non-OTel-standardized key. `ATTR_LOCK_ACQUIRED` was genuinely never emitted and is now set on the acquire span (`True` on success, `False` when acquisition fails), so all four lock constants have a real write site. Dashboards keyed on the old unprefixed `lock.*` attributes need updating; emitting both spellings was rejected as reinstating the divergence. Recorded in `docs/adrs/0064-telemetry-attribute-catalogue-is-not-a-wishlist.md`, which amends ADR 0016.
- **`LeaderElectorWithLease` is deleted; `LeaderElector` stays.** It extended the base Protocol with `lease_duration_seconds`, `lease_remaining_seconds`, and `wait_for_leadership()`. Nothing in the tree implemented it, nothing consumed those three members, and the library drives none of the lifecycle they describe: it touches an elector at exactly three points -- reads `is_leader` when building a heartbeat, reads it again to gate a leadership handoff, and calls `release()` during that handoff. It never calls `try_acquire()` and never calls `renew()`, so acquisition is the user's and renewal travels with acquisition. The split was made in ADR 0009 expecting `KubernetesLeaderElector` and `RedisLeaderElector`, and neither arrived; a backend with leases now defines its own lease API, which beats conforming to a shape this library guessed at without ever calling it. No behavior changes -- nothing called these members. Code type-hinting `LeaderElectorWithLease` or calling `wait_for_leadership()` through it must switch to its own type. Recorded in `docs/adrs/0061-leader-lease-protocol-deleted.md`, which amends ADR 0009 (the lease split only).
...
- **`SubscriptionConfig.shutdown_timeout` is deleted.** It defaulted, validated, and read back correctly, and nothing in the tree ever read it: `SubscriptionManager`'s own `shutdown_timeout` constructor argument was what reached the `ShutdownCoordinator`, so a user who set the field on the config and expected it to govern shutdown got the manager's default instead, silently. One fact declared in two places with nothing failing when the copies disagreed. Set the timeout on `SubscriptionManager`, which is where it was always read from. A regression test asserts the attribute is gone and that passing the keyword raises, so re-adding the second site fails loudly. No shim, per the pre-1.0 no-shim policy. Recorded in `docs/adrs/0062-single-declaration-sites-for-shutdown-timeout-and-retry-policy.md`.
- **The projections retry Protocol is renamed from `RetryPolicy` to `ProjectionRetryPolicy`.** Two unrelated types shared the bare name: this Protocol (`delay_for`) in `eventsource.application.projections.retry`, and the bus backoff dataclass (`max_retries`/`get_backoff`/`should_retry`) that `from eventsource import RetryPolicy` actually resolves to. They are structurally incompatible, so a user who followed the top-level export into a projection's `retry_policy` parameter handed the projection loop a type it cannot call. The bus dataclass keeps both the name and the export; only the Protocol is renamed. Code importing `RetryPolicy` from `eventsource.application.projections.retry` (or type-hinting a `retry_policy` argument with it) must switch to `ProjectionRetryPolicy`. No shim, per the pre-1.0 no-shim policy. Recorded in the same ADR 0062.
...
### Added
- **`generate_additive_migration()` and `reconcile_read_model_schema()` add the columns a `ReadModel` gained to a table that already exists.** `generate_schema()` emits `CREATE TABLE IF NOT EXISTS`, which does nothing whatsoever to a table that is already there and does not report that it did nothing — so adding a field to a read model was silently a no-op against every database created before that field existed, and the projection then wrote a column that was not there. No test could catch it: tests build their tables from nothing, where the `CREATE` is always complete, so the gap opened exactly between the environments that are cheap to test and the one that is not. The library already did this for its own tables (`adapters/sql/schemas/`) and offered consumers nothing. `generate_additive_migration(model_class, existing_columns, dialect)` in `eventsource.adapters.sql.readmodel_schema` is pure — current columns in, `ALTER TABLE ... ADD COLUMN` statements out — so the diff can be inspected or handed to a migration tool without being executed. `reconcile_read_model_schema(conn, model_class)` in `eventsource.adapters.sql.readmodel_reconcile` introspects a live connection (`information_schema.columns` on PostgreSQL, `pragma_table_info` on SQLite), executes the statements, and returns what it ran; it takes an `AsyncConnection` (your transaction) or an `AsyncEngine` (one is opened and committed), creates the table with its indexes when it does not exist, and is idempotent. It takes no dialect argument — the connection carries that fact. **Additive only and opt-in**: nothing is dropped, retyped, renamed, or reordered; a column the model no longer declares is left alone; indexes on an existing table are not reconciled; and nothing in the library calls it, so it does not compete with Alembic. A required field with no default raises the new `ReadModelSchemaMismatchError` (a `ReadModelError`, exported from `eventsource.ports.readmodels`) *before* any statement executes, since it cannot be added to a table that may already have rows. Recorded in `docs/adrs/0066-read-model-schema-reconciliation-is-additive-and-opt-in.md`, which extends ADR 0039.
...
- **The live subscription runner now dispatches through `handle_batch()` for subscribers that implement it**, completing the batch-delivery path that ADR 0059 sanctioned as the way to go faster without concurrent completion. `handle_batch()` takes precedence over `handle()` on the live path exactly as it already did on catch-up. **The batch is a page the feed already returned, never an accumulator**: the runner dispatches exactly what was available past the checkpoint (bounded by `config.batch_size`, the same read it already issued), so nothing is held back waiting for more events, a lone event is still delivered immediately, and there is no window, timer, or minimum batch size. Materializing the page costs no latency either, since feed adapters build their whole result set before yielding the first envelope. Stop and pause stay **per envelope** — the page is scanned before dispatch, making the same `_stop_requested`/`wait_if_paused()` checks the per-event path made — so the interruption guarantees are unchanged. What coarsens is the window inside one handler call, which is `processing_timeout`: it bounds one `handle_batch()` just as it bounds one `handle()`, so a large live batch shares one budget. If `handle_batch()` raises, delivery falls back to the per-event path with the position recorded after each envelope, per `BatchSubscriber.handle_batch`'s documented contract. `EVERY_BATCH` checkpointing acquires a meaning on the live path that it did not previously have. A time-window accumulator was rejected: it buys throughput with the live path's defining property, and a subscription that cannot keep up live is a catch-up problem. Recorded in `docs/adrs/0063-live-batch-delivery-is-a-page-not-a-window.md`.
...
[Unreleased]: https://github.com/tyevans/eventsource-py/compare/v0.14.0...HEAD
[0.14.0]: https://github.com/tyevans/eventsource-py/compare/v0.13.0...v0.14.0
