...
- **The PostgreSQL adapter gained same-transaction outbox write support** (`PostgreSQLEventStore(..., outbox_enabled=True)`), so the transactional-outbox guarantee that the legacy `PostgreSQLEventStore` provided survives the deletion of its only previous writer.
- **`SubscriptionPositions` and `CheckpointData` now carry the opaque `Position` value object rather than an integer global position** (amends ADR 0024; see ADR 0025).
- **Migration lag is now count-behind, not a position delta**, completing the amendment ADR 0019 already made to ADR 0014 (opaque positions cannot be subtracted). See ADR 0025.
